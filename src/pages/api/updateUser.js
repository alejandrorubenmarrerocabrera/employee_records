...
export default async function handler(req, res) {
	if (req.method === 'POST') {
		try {
			...
			const SQL = 'UPDATE employees SET first_name = ?, last_name = ?, birthday = ? WHERE employee_id = ?';

			const [result] = await pool.query(SQL, [first_name, last_name, birthday, employee_id]);
